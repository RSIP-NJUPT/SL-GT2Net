...
class Resize(object):
    ...
    def _resize_img(self, results):
        ...
        if results["img"].ndim == 4:  # for 4d data, D,H,W,C
            imgs = []
            if self.keep_ratio:
                for img in results["img"]:
                    tmp_img, scale_factor = mmcv.imrescale(img, results["scale"], return_scale=True)
                    imgs.append(tmp_img)
                # the w_scale and h_scale has minor difference
                # a real fix should be done in the mmcv.imrescale in the future
                new_h, new_w = tmp_img.shape[:2]
                h, w = results["img"].shape[1:-1]  # T,H,W,C
                w_scale = new_w / w
                h_scale = new_h / h
            else:
                for img in results["img"]:
                    tmp_img, w_scale, h_scale = mmcv.imresize(img, results["scale"], return_scale=True)
                    imgs.append(tmp_img)
        else:
            if self.keep_ratio:
...
class RandomFlip(object):
    ...
    def __call__(self, results):
        ...
        if results["flip"]:
            # flip image
            if results["img"].ndim == 4:
                imgs = [mmcv.imflip(img, direction=results["flip_direction"]) for img in results["img"]]
                results["img"] = np.stack(imgs, axis=0)
            else:
...
class Pad(object):
    ...
    def __init__(self, size=None, size_divisor=None, pad_val=0, seg_pad_val=255, num_bands=None, mode='constant'):
        self.size = size
    ...
    def _pad_img(self, results):
        ...
        if self.size is not None:
            ...
            if results["img"].ndim == 4:
                padded_imgs = [mmcv.impad(img, shape=self.size, pad_val=self.pad_val) for img in results["img"]]
                results["img"] = np.stack(padded_imgs, axis=0)
            else:
                results["img"] = mmcv.impad(results["img"], shape=self.size, pad_val=self.pad_val)
        elif self.size_divisor is not None:  # not use, not modify!
    ...
    def _pad_img_bands(self, results):
        """Pad images according to ``self.num_bands``."""
        # t h w c for slovenia and brandenburg, pastis-r
        if self.num_bands is not None:
            if results["img"].ndim == 4: #
                img = results['img']
                T, H, W, C = img.shape
                padding = self.num_bands - T
                padded_imgs = np.pad(img, ((padding//2, padding - padding //2),(0,0),(0,0),(0,0)), mode='reflect')
                results["img"] = padded_imgs
                results["num_bands"] = self.num_bands
    ...
    def _pad_seg(self, results):
        ...
        for key in results.get("seg_fields", []):
            results[key] = mmcv.impad(
                results[key], shape=results["pad_shape"][1:-1], pad_val=self.seg_pad_val  # T,H,W,C
            )
    ...
    def __call__(self, results):
        ...
        Returns:
            dict: Normalized results, 'img_norm_cfg' key is added into
                result dict.
        """
        # TODO ====试试两个通道的数据keyi使用imnormalize
        # 经过测试两个通道也可，或者说任意通道都可，前提是mean/std.dtype=np.float64（np.float32报错!!!）
        # results['img'] = mmcv.imnormalize(results['img'], self.mean, self.std,
        #                                   self.to_rgb)
        if results["img"].ndim == 4:
            imgs = [mmcv.imnormalize(img, self.mean, self.std, self.to_rgb) for img in results["img"]]
            results["img"] = np.stack(imgs, axis=0)
        else:  # results['img'].ndim = 3
...
class RandomCrop(object):
    ...
    def __call__(self, results):
        ...
        crop_bbox = self.get_crop_bbox(seg_img)
    ...
    def __call__(self, results):
        ...
        if rotate:
            ...
            results["sequence_imgs"] = [
                mmcv.imrotate(
                    img, angle=degree, border_value=self.pal_val, center=self.center, auto_bound=self.auto_bound
                )
...
class RGB2Gray(object):
    ...
    def __call__(self, results):
        ...
        img = self.rgb2gray(img, weights)

        results["img"] = img
        results["img_shape"] = img.shape

        # change sequence
        results["sequence_imgs"] = [self.rgb2gray(img, weights) for img in results["sequence_imgs"]]
