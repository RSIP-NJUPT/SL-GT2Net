...
class SloveniaDataset(Dataset):
    # todo
    """Custom dataset for semantic segmentation. An example of file structure
    is as followed.

    slovenia DATASET: http://gpcv.whu.edu.cn/data/dataset12/dataset12.html
    ...
    def __init__(
        ...
        # load annotations
        self.img_infos = self.load_annotations(self.img_dir, self.img_suffix, self.ann_dir, self.seg_map_suffix)
    ...
    def evaluate(self, results, metric="mIoU", logger=None, **kwargs):
        ...
        # todo ==================
        # samples = np.zeros((9), dtype=np.uint32)  # 0-8
        # for data in gt_seg_maps:
        #     for i in np.unique(data):
        #             if i != 255:
        #                 samples[i+1] += np.sum(data == i)
        #             else:
        #                 samples[0] += np.sum(data==i)
        # for i in range(len(samples)):
        #     print("class_{} : {} samples".format(i, samples[i]))
        # todo =====================
        ...
        ret_metrics = eval_metrics(results, gt_seg_maps, num_classes, ignore_index=self.ignore_index, metrics=metric)
        ...
        summary_table_data = [["Scope"] + ["aAcc"] + ["m" + head for head in class_table_data[0][1:]]]
