...
def eval_metrics(results,
    ...
    total_area_intersect, total_area_union, total_area_pred_label, \
        total_area_label = total_intersect_and_union(results, gt_seg_maps,
                                                     num_classes,
                                                     ignore_index=ignore_index)
    all_acc = total_area_intersect.sum() / total_area_label.sum() # OA
    acc = total_area_intersect / total_area_label   # TP / (TP + FN), recall
    precision = total_area_intersect / total_area_pred_label # TP / (TP + FP)
    ret_metrics = [all_acc, acc, precision]
    for metric in metrics: # todo
        if metric == 'mIoU':
            iou = total_area_intersect / total_area_union
            ret_metrics.append(iou) # ret_metrics = [all_acc, acc, precision, iou]
        elif metric == 'mF1score':
            # acc is recall
            recall = acc
            # precision = total_area_intersect / total_area_pred_label
            f1score = 2*(recall * precision) / (recall + precision)
            ret_metrics.append(f1score) # ret_metrics = [all_acc, acc, precision, iou, f1score]
        elif metric == 'mDice':
            dice = 2 * total_area_intersect / (
                total_area_pred_label + total_area_label)
            ret_metrics.append(dice) # # ret_metrics = [all_acc, acc, precision, iou, f1score, dice]
